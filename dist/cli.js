...
import { CanopyApiClient, CanopyApiError } from "./api.js";
...
async function main() {
    ...
    // 1. Pick starter
    const sortedStarters = [...STARTERS].sort((a, b) => Number(b.recommendedFirst ?? false) - Number(a.recommendedFirst ?? false));
    const starterSlug = await select({
    ...
    console.log("");
    info(`Connect to your Canopy org. You can find your API key here:\n   ${dim("https://trycanopy.ai/dashboard/settings#api-keys")}`);
    const apiKey = (await password({
        message: "Org API key (ak_live_…):",
        validate: (v) => v.trim().startsWith("ak_") || v.trim().startsWith("ak_live_")
            ? true
    ...
    if (!org.treasury_provisioned) {
        fail(`Org "${org.org_name ?? org.org_id}" has no treasury provisioned yet. Open the dashboard once to finish setup, then re-run this command.`);
        process.exit(1);
    }
    success(`Connected to "${bold(org.org_name ?? org.org_id)}" (treasury ${dim(shortenAddress(org.treasury_address))})`);
    // 3. Agent name
    ...
    // 8. Anthropic API key
    console.log("");
    info(`Claude Agent SDK runs on Claude. Get a key here:\n   ${dim("https://console.anthropic.com/")}`);
    ...
    info(`To restrict which services this agent can pay (allowlist), browse the\n   registry and pick services in the policy editor in the dashboard.`);
    if (!anthropicKey) {
        warn(`Don't forget to set ANTHROPIC_API_KEY in ${path.join(projectName, ".env")} before \`npm start\`.`);
    }
}
...
function printPolicyTable(starter) {
    ...
    console.log(`   • Allowlisted services configure in dashboard → https://trycanopy.ai/dashboard`);
}
async function pickApproval(starter) {
