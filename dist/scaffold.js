...
export function templatesRoot() {
    ...
    // SELF_DIR for source dev: <repo>/create-canopy-agent/src
    // SELF_DIR for built CLI: <pkg>/dist
    const sourceFallback = path.resolve(SELF_DIR, "..", "..", "canopy-agent-starters");
...
export async function scaffold(args) {
    ...
    if (existingEntries.length > 0) {
        throw new Error(`Destination ${args.destDir} is not empty. Pick a fresh path or remove it first.`);
    }
...
async function rewritePackageJson(dir, projectName) {
